...
const images = [
  ...
  {
    filename: 'public/images/brands/stellar.webp',
    width: 900,
    height: 600,
    title: 'Stellar',
    colors: ['#0f172a', '#111827', '#1f2937'],
...
function buildSvg({ width, height, colors, accent, title, nodes, grid }) {
  ...
  const nodeElements = nodes
    ? Array.from({ length: 16 }, (_, i) => {
        const x = 40 + ((i * 67) % (width - 120));
        const y = 40 + ((i * 43) % (height - 120));
        const r = 6 + ((i % 4) * 1.6);
        return `<circle cx='${x}' cy='${y}' r='${r}' fill='${accent}' opacity='0.7'/>`;
      }).join('')
    : '';
  ...
  <svg width='${width}' height='${height}' viewBox='0 0 ${width} ${height}' xmlns='http://www.w3.org/2000/svg'>
    <defs>
      ...
      <radialGradient id='spot' cx='50%' cy='15%' r='60%'>
        <stop offset='0%' stop-color='rgba(255,255,255,0.18)' />
        <stop offset='100%' stop-color='rgba(255,255,255,0)' />
      </radialGradient>
    </defs>
    <rect x='0' y='0' width='${width}' height='${height}' fill='url(#grad)' />
    <rect x='0' y='0' width='${width}' height='${height}' fill='url(#spot)' opacity='0.34' />
    ${grid ? lines : ''}
    ${nodeElements}
    <rect x='${width * 0.12}' y='${height * 0.2}' width='${width * 0.28}' height='${height * 0.24}' rx='32' fill='rgba(255,255,255,0.08)' stroke='${accent}' stroke-width='2' />
    <rect x='${width * 0.16}' y='${height * 0.28}' width='${width * 0.2}' height='${height * 0.035}' rx='12' fill='rgba(255,255,255,0.12)' />
    <rect x='${width * 0.16}' y='${height * 0.34}' width='${width * 0.22}' height='${height * 0.035}' rx='12' fill='rgba(255,255,255,0.12)' />
    <circle cx='${width * 0.8}' cy='${height * 0.24}' r='50' fill='rgba(255,255,255,0.05)' stroke='${accent}' stroke-width='3' />
    <circle cx='${width * 0.75}' cy='${height * 0.75}' r='62' fill='rgba(255,255,255,0.04)' />
    <circle cx='${width * 0.82}' cy='${height * 0.78}' r='28' fill='${accent}' opacity='0.18' />
    <rect x='${width * 0.08}' y='${height * 0.7}' width='${width * 0.19}' height='${height * 0.1}' rx='20' fill='rgba(255,255,255,0.1)' />
    ${title ? `<text x='${width * 0.08}' y='${height * 0.16}' fill='rgba(255,255,255,0.92)' font-family='Inter, sans-serif' font-size='40' font-weight='700'>${title}</text>` : ''}
  </svg>
  `;
}
